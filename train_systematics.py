...
def _residual_kwargs(res_cfg: Dict[str, Any]) -> Dict[str, Any]:
    """Optional SystematicCorrectedModel kwargs read from a residual_*_model config.

    Enables nuisance cross-terms (the m_i·m_j coupling absorbed by IndependentPolynomial
    ResidualTransform.cross_nets) and lets the polynomial dampings be set from YAML:

        residual_kin_model:
          ...
          cross_term_pairs: [[0, 1]]      # add the ν_shift·ν_squeeze cross-term
          cross_term_damping: 0.01        # optional (default 0.01)
          quadratic_damping: 0.01         # optional (default 0.01)

    Keys are forwarded only when present, so configs WITHOUT them keep the model's
    defaults (no cross-terms) and stay identical to the additive v15 residuals.
    """
    kwargs: Dict[str, Any] = {}
    pairs = res_cfg.get("cross_term_pairs")
    if pairs:
        kwargs["cross_term_pairs"] = [tuple(int(i) for i in p) for p in pairs]
    if "cross_term_damping" in res_cfg:
        kwargs["cross_term_damping"] = float(res_cfg["cross_term_damping"])
    if "quadratic_damping" in res_cfg:
        kwargs["quadratic_damping"] = float(res_cfg["quadratic_damping"])
    return kwargs
...
def _train_kin(cfg: Dict[str, Any], device: str) -> None:
    ...
    model = SystematicCorrectedModel(
        ...
        type=res_cfg["type"],
        **_residual_kwargs(res_cfg),
    ).to(device)
    if res_cfg.get("cross_term_pairs"):
        print(f"  cross-terms enabled: cross_term_pairs={res_cfg['cross_term_pairs']}")
    ...
    exp = None
    if exp is not None:
        exp.log_parameters({"residual": "kin", "device": device, "batch_size": batch_size,
                            "nepochs": nepochs, "steps_per_epoch": steps_per_epoch, "lr": lr,
                            "num_nuisances": n_nuis, "n_generators": len(generators)})
    global_step = 0
    ...
    for epoch in range(nepochs):
        ...
        for step in range(steps_per_epoch):
            ...
            if exp is not None and global_step % 100 == 0:
                exp.log_metrics({"kin/loss_step": float(loss.item()),
                                 "kin/lr": float(scheduler.get_last_lr()[0])},
        ...
        if exp is not None:
            exp.log_metric("kin/loss_epoch", float(avg), step=global_step, epoch=epoch)
...
def _train_score(cfg: Dict[str, Any], device: str) -> None:
    ...
    exp = None
    if exp is not None:
        exp.log_parameters({"residual": "score", "device": device, "batch_size": batch_size,
                            "nepochs": nepochs, "steps_per_epoch": steps_per_epoch, "lr": lr,
                            "num_nuisances": n_nuis, "n_generators": len(generators)})
    global_step = 0
    ...
    for epoch in range(nepochs):
        ...
        for step in range(steps_per_epoch):
            ...
            train_loss += loss.item()
            if exp is not None and global_step % 100 == 0:
                exp.log_metrics({"score/loss_step": float(loss.item()),
                                 "score/lr": float(scheduler.get_last_lr()[0])},
                                step=global_step, epoch=epoch)
            global_step += 1
            if step % 500 == 0:
                print(f"  epoch={epoch} step={step}/{steps_per_epoch} loss={loss.item():.4f}")
        ...
        print(f"  epoch={epoch} avg_loss={avg:.4f}")
        if exp is not None:
            exp.log_metric("score/loss_epoch", float(avg), step=global_step, epoch=epoch)
